...
# Dynamic tensorboard folder based on traffic_file
dataset_name = os.path.basename(os.path.dirname(args.traffic_file))
tensorboard_folder = f'./runs/Multitask_Stockformer/{dataset_name}'
...
def test_res(model, valXL, valXH, valXC, bonus_valX, valTE, valY, valYC, adjgat):
    ...
    # 保存分类任务的最后一个时间步的预测和标签 - use dynamic output path
    dataset_name = os.path.basename(os.path.dirname(args.traffic_file))
    output_base = f'./output/Multitask_output_{dataset_name.replace("Stock_CN_", "")}'
    os.makedirs(f'{output_base}/classification', exist_ok=True)
    os.makedirs(f'{output_base}/regression', exist_ok=True)
    
    save_to_csv(f'{output_base}/classification/classification_pred_last_step.csv', pred_class[:, -1, :])
    save_to_csv(f'{output_base}/classification/classification_label_last_step.csv', label_class[:, -1])

    # 保存回归任务的最后一个时间步的预测和标签
    save_to_csv(f'{output_base}/regression/regression_pred_last_step.csv', pred_regress[:, -1, :])
    save_to_csv(f'{output_base}/regression/regression_label_last_step.csv', label_regress[:, -1])
