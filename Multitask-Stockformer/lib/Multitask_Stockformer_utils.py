...
class StockDataset(Dataset):
    def __init__(self, args, mode='train'):
        ...
        # Dynamically extract Alpha_360 path from traffic_file
        import os
        data_dir = os.path.dirname(args.traffic_file)
        dataset_name = os.path.basename(data_dir)
        alpha_folder_name = f"Alpha_360_{dataset_name.replace('Stock_CN_', '')}"
        path = os.path.join(data_dir, alpha_folder_name)
